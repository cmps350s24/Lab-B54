...
async function loadContentFromServer(url) {
    try {
         ...
         <li class="cards__item" id="card-${book.isbn}">
            <div class="card">
                ...
                <div class="card__content">
                    ...
                    <p id="book-desc" class="card__text">${book.shortDescription ? book.shortDescription.trim().substring(0, 300) + "...." : 'Not Available'}</p>
...
async function handleUpdateBook(isbn) {
    try {
        ...
        const book = await loadContentFromServer(url);
        if (!book) {
...
function populateBookForm(book) {
    console.log(book);
    let publishedDate = new Date(book.publishedDate).toISOString().split('T')[0];
    ...
    document.querySelector("#publishedDate").value = publishedDate;
...
async function handleBookSearch(event) {
    try {
        ...
        console.log(url);
        const books = await loadContentFromServer(url);
        console.log(books);
        ...
        document.querySelector("#main-content").innerHTML = `
            ...
            <li class="cards__item" id="card-${book.isbn}">
                <div class="card">
                    ...
                    <div class="card__content">
                        <div id="book-title" class="card__title">${book.title}</div>
                        <p id="book-desc" class="card__text">${book.shortDescription ? book.longDescription.trim() : 'Not Available'}</p>
                        <div class="btn--options">
                            <button class="btn btn--update" onclick="handleUpdateBook('${book.isbn}')">Update</button>
                            <button class="btn btn--delete" onclick="handleDeleteBook('${book.isbn}')">Delete</button>
                        </div>
